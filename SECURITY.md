...
- **OS package findings the image cannot reach.** The container image upgrades its Alpine
  packages at build time and then removes the package manager, which takes OpenSSL with it
  — Bun brings its own TLS, so nothing CI Deck runs ever used it. A scan of a freshly built
  image shows nothing Alpine has patched. Alpine does still list
  [CVE-2025-60876](https://security.alpinelinux.org/vuln/CVE-2025-60876) as unfixed on
  every branch — request splitting through BusyBox's `wget` applet — and a scanner may
  report it. Nothing in the image runs `wget`: the health check is `bun -e` and `fetch`,
  and the only subprocesses CI Deck ever starts are the macOS and Windows credential
  stores. Please do report an OS package finding that something in CI Deck actually calls.
- **Findings in the development toolchain.** TypeScript's compiler is a native Go binary,
  and a scan of `node_modules` reports the Go standard-library CVEs of the toolchain it was
  built with. It only type-checks this repository's own source, and nothing that is
  published carries it — not the package, the standalone binaries or the image.
